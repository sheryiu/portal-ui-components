...
import {
  ACTION_DRAWER_LAYOUT_DATA_PROVIDER,
  ActionDrawerOverlayService,
  LayoutControlDirective,
  TABLE_CONTENT_DATA_PROVIDER,
} from 'portal-ui-ng/pages';
...
export class InventoryItemInfoComponent {
  ...
  customer = computed(() => {
    return this.customerList()?.find(
      (c) => c.id == this.inventoryItem()?.belongsTo,
    );
  });
  shelf = computed(() => {
    return this.inventoryShelfList()?.find(
      (s) => s.id == this.inventoryItem()?.locatedIn,
    );
  });
  showEditButton = computed(() => {
    return [InventoryItemStatus.OPEN, InventoryItemStatus.REPACKAGING].includes(
  ...
  onSaveClick() {
    this.dataService.save(this.id()!, {
      ...this.inventoryItem()!,
      belongsTo:
        this.updatedValues().customer?.id ?? this.inventoryItem()!.belongsTo,
    });
    this.isEditing.set(false);
    this.updatedValues.set({
      customer: null,
  ...
  onUpdateCustomerClick() {
    this.actionDrawer.open(CustomerPickerService, {
      providers: [
        {
      ...
      overlayData: {
        initialValue: new Set(
          [this.updatedValues().customer ?? this.customer()].filter(isNonNull),
        ),
  ...
  onRemoveClick() {
    const ref = this.modalDialog.open({
      ...
      onDetailsComponentAttached: (remove) => {
        ...
        remove.instance.matchChanged.subscribe((m) => {
          // TODO remove
        });
      },
      actions: [
