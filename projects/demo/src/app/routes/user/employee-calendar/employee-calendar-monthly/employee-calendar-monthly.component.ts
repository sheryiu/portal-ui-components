...
export class EmployeeCalendarMonthlyComponent {
  ...
  protected daysVisible = computed(() => {
    ...
    return eachDayOfInterval({ start, end }).map((date) => ({
      date,
      isLastMonth:
        getMonth(date) < getMonth(selectedTime)
          ? true
          : getYear(date) < getYear(selectedTime),
      isNextMonth:
        getMonth(date) > getMonth(selectedTime)
          ? true
          : getYear(date) > getYear(selectedTime),
  ...
  protected daysWithData = computed(() => {
    ...
    return days.map((day) => {
      return {
        ...day,
        events: rawData
          .filter((event) => isSameDay(event.startsFrom, day.date))
          .sort((a, b) => a.startsFrom.getTime() - b.startsFrom.getTime()),
      };
    });
  });
