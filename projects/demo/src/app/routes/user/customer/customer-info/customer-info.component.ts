...
export class CustomerInfoComponent
  implements TableContentDataProvider<InventoryItem>
{
  ...
  inventoryItems = computed(() => {
    return (
      this.allInventoryItems()?.filter(
        (item) => item.belongsTo == this.customer()?.id,
      ) ?? []
    );
  });
  awaitingPaymentsCount = computed(
    () =>
      this.inventoryItems()?.filter((i) => i.status == InventoryItemStatus.OPEN)
        .length,
  );
  repackageCount = computed(
    () =>
      this.inventoryItems()?.filter(
        (i) => i.status == InventoryItemStatus.REPACKAGING,
      ).length,
  );
  data = computed(() => {
