...
@Component({
  ...
  imports: [TimeDisplayComponent, ButtonModule, NgClass],
  templateUrl: './system-log-chart.component.html',
})
export class SystemLogChartComponent {
  private injector = inject(Injector);
  private chartCanvas: Signal<ElementRef<HTMLCanvasElement>> =
    viewChild.required('chart', { read: ElementRef });
  ...
  constructor() {
    afterNextRender(() => {
      ...
      effect(
        () => {
          this.chart.data.labels = eachDayOfInterval({
            start: this.startDate(),
            end: this.endDate(),
          }).map((day) => format(day, 'd MMM'));
          // Errors
          if (this.showErrors()) {
            if (this.chart.data.datasets.some((d) => d.label == 'Errors')) {
              this.chart.data.datasets.find((d) => d.label == 'Errors')!.data =
                eachDayOfInterval({
                  start: this.startDate(),
                  end: this.endDate(),
                }).map(
                  (day) =>
                    this.errors().filter((l) => isSameDay(l.timestamp, day))
                      .length,
                );
            } else {
            ...
            !this.showErrors() &&
            this.chart.data.datasets.some((d) => d.label == 'Errors')
          ) {
            this.chart.data.datasets.splice(
              this.chart.data.datasets.findIndex((d) => d.label == 'Errors'),
              1,
            );
          }
          // Warnings
          if (this.showWarns()) {
            if (this.chart.data.datasets.some((d) => d.label == 'Warnings')) {
              this.chart.data.datasets.find(
                ...
                (day) =>
                  this.warns().filter((l) => isSameDay(l.timestamp, day))
                    .length,
              );
            } else {
              this.chart.data.datasets.splice(0, 0, {
            ...
            !this.showWarns() &&
            this.chart.data.datasets.some((d) => d.label == 'Warnings')
          ) {
            this.chart.data.datasets.splice(
              this.chart.data.datasets.findIndex((d) => d.label == 'Warnings'),
              1,
            );
          }
          // Infos
          if (this.showInfos()) {
            if (this.chart.data.datasets.some((d) => d.label == 'Infos')) {
              this.chart.data.datasets.find((d) => d.label == 'Infos')!.data =
                eachDayOfInterval({
                  start: this.startDate(),
                  end: this.endDate(),
                }).map(
                  (day) =>
                    this.infos().filter((l) => isSameDay(l.timestamp, day))
                      .length,
                );
            } else {
              this.chart.data.datasets.splice(0, 0, {
            ...
            !this.showInfos() &&
            this.chart.data.datasets.some((d) => d.label == 'Infos')
          ) {
            this.chart.data.datasets.splice(
              this.chart.data.datasets.findIndex((d) => d.label == 'Infos'),
              1,
            );
          }
          // Debugs
          if (
            this.showDebugs() &&
            !this.chart.data.datasets.some((d) => d.label == 'Debugs')
          ) {
            if (this.chart.data.datasets.some((d) => d.label == 'Debugs')) {
              this.chart.data.datasets.find((d) => d.label == 'Debugs')!.data =
                eachDayOfInterval({
                  start: this.startDate(),
                  end: this.endDate(),
                }).map(
                  (day) =>
                    this.debugs().filter((l) => isSameDay(l.timestamp, day))
                      .length,
                );
            } else {
              this.chart.data.datasets.splice(0, 0, {
            ...
            this.chart.data.datasets.splice(
              this.chart.data.datasets.findIndex((d) => d.label == 'Debugs'),
              1,
            );
          }
          this.chart.data.datasets.sort(
            (a, b) =>
              CHART_ORDER.indexOf(a.label!) - CHART_ORDER.indexOf(b.label!),
          );
          this.chart.update();
        },
