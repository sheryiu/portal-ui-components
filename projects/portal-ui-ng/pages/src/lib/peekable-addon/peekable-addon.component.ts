...
@Component({
  ...
  imports: [RouterOutlet, RouterLink, ButtonModule, TooltipDirective],
...
export class PeekableAddonComponent {
  private dataProvider = inject(PEEKABLE_ADDON_DATA_PROVIDER, {
    optional: true,
  });
  ...
  protected routeToFullContent = computed(
    () => this.dataProvider?.routeToFullContent() ?? null,
  );
  ...
  constructor() {
    ...
    this._keydownEvents.pipe(takeUntilDestroyed()).subscribe((event) => {
      if (event.key == 'Escape') {
  ...
  onEscapePress(event: Event) {
    if (
      this.route.snapshot.parent?.children.some(
        (child) => child.outlet == 'peek',
      )
    ) {
      this.router.navigate([{ outlets: { peek: [] } }], {
