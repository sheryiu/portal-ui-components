...
import {
  EDITABLE_CONTENT_DATA_PROVIDER,
  EDITABLE_CONTENT_DEFAULT_CONTROLS,
  EditableContentDataProvider,
} from './editable-content';

@Component({
  selector: 'pui-editable-content',
  imports: [FieldModule, LayoutControlDirective],
...
> implements PreventDirtyLeaveComponent
{
  private route = inject(ActivatedRoute);
  private dataProvider = inject(
    EDITABLE_CONTENT_DATA_PROVIDER,
  ) as EditableContentDataProvider<T>;

  protected data = linkedSignal(() => this.dataProvider.data());
  protected fieldConfiguration = computed(() =>
    this.dataProvider.fieldConfiguration(),
  );
  ...
  protected controlsConfig = computed(() => {
    return (
      this.dataProvider.controlsConfig?.() ?? EDITABLE_CONTENT_DEFAULT_CONTROLS
    );
  });
