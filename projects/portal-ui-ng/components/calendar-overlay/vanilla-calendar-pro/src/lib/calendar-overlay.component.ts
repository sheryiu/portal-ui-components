...
export class CalendarOverlayComponent {
  ...
  private divElement: Signal<ElementRef<HTMLDivElement>> =
    viewChild.required('calendar');
  ...
  private onChange() {
    if (this.data.dateOnly) {
      this.data.onDateChange(
        this.calendar.context.selectedDates.length > 0
          ? new Date(`${this.calendar.context.selectedDates[0]} 00:00:00`)
          : null,
      ); // this gets converted to the selectedTime in local timezone
    } else {
      this.data.onDateChange(
        this.calendar.context.selectedDates.length > 0
          ? new Date(
              `${this.calendar.context.selectedDates[0]} ${this.calendar.context.selectedTime}`,
            )
          : null,
      ); // this gets converted to the selectedTime in local timezone
    }
