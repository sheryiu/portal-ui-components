      ...
      @if (resizable()) {
        @if (!$last) {
          <button
            ...
            [style.translate]="
              selectedResizeHandle() == def.columnName
                ? resizeHandleTranslate()
                : undefined
            "
          ></button>
        }
...
export class TableHeaderRowComponent {
  ...
  cells = computed(() => {
    ...
    return columns
      .map((columnName) =>
        cellDefs.find((def) => def.columnName() == columnName),
      )
      .map((def, i) =>
        def
          ? {
  ...
  protected resizeHandleTranslate = computed(
    () => `${this.resizeHandleDX()}px 0px`,
  );
  ...
  onResizeHandleUp(event: PointerEvent, columnName: string, index: number) {
    const previous = (event.currentTarget as HTMLElement)
      .previousElementSibling;
    if (previous) {
      const newSize =
        previous.getBoundingClientRect().width + this.resizeHandleDX();
