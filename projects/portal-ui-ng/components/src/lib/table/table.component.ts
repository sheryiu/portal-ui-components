...
export class TableComponent {
   ...
   */
  inputColumns = input.required<
    Record<'default' | number, string[]> | string[]
  >({ alias: 'columns' });
  inputColumnWidths = input<Record<'default' | number, string[]> | string[]>(
    undefined,
    { alias: 'columnWidths' },
  );
  ...
  columnResized = output<{
    resizedKey: 'default' | number;
    newColumnWidths: string[];
  }>();
  ...
  private storedResizedColumnWidths = signal<Record<
    'default' | number,
    string[]
  > | null>(null);
  ...
  private activeColumnWidths = linkedSignal(
    () => this.columnWidths()[this.activeColumnWidthsKey()],
  );
  visibleColumnWidths = computed(() => {
    ...
    if (visible.length == columns.length)
      return visible.toSpliced(visible.length - 1, 1, 'minmax(80px, 1fr)');
    return visible.toSpliced(
      visible.length,
      0,
      ...Array(columns.length - visible.length).fill('minmax(80px, 1fr)'),
    );
  });
    ...
    effect(() => {
      ...
      if (id != null && inputColumnWidths != null && this.storage != null) {
        if (this.storage.getItem(`pui-table.${id}`)) {
          const { key, resized } = JSON.parse(
            this.storage.getItem(`pui-table.${id}`)!,
          );
