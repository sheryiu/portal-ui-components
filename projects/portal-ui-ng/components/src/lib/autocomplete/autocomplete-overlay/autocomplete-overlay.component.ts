...
@Component({
  ...
  imports: [NgTemplateOutlet, A11yModule, ButtonModule],
...
export class AutocompleteOverlayComponent<D> {
  ...
  protected items = this.data.items.map(
    (item) => [item, this.data.valueFn(item)] as const,
  );
  private selectedIndex = signal(
    this.items.findIndex(([_, v]) => v == this.data.selectedValue),
  );
  ...
  private prevOption() {
    ...
    const at = this.options().findIndex(
      (el) => el.nativeElement === this.selectedOption(),
    );
    if (at - 1 >= 0) {
