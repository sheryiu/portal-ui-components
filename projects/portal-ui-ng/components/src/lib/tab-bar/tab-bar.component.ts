...
@Component({
  ...
  imports: [NgTemplateOutlet, HoverableDirective],
  providers: [
    {
...
export class TabBarComponent implements AfterViewInit, ControlValueAccessor {
  ...
  private updateStyle(currentTab: TabDirective | undefined) {
    ...
    const el = this.tabButtonsElement().find(
      (el) => el.nativeElement.id === currentTab.id(),
    );
    if (el?.nativeElement) {
      this.hostSelectedTabWidth.set(
        el!.nativeElement.getBoundingClientRect().width,
      );
      this.hostSelectedTabHeight.set(
        el!.nativeElement.getBoundingClientRect().height,
      );
