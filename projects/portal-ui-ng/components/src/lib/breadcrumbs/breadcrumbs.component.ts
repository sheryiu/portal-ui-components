...
@Component({
  ...
  imports: [AsyncPipe, RouterLink, HoverableDirective, TooltipDirective],
...
export class BreadcrumbsComponent {
  ...
  constructor() {
      ...
      if (data) {
        ...
        if (breadcrumb) {
          breadcrumbs.unshift({
            ...
            injectedDeps:
              'deps' in breadcrumb
                ? breadcrumb.deps?.map((d) => inject(d))
                : [],
            route: parent,
          });
        }
    ...
    this.breadcrumbs$ = of(breadcrumbs).pipe(
      map((breadcrumbs) =>
        breadcrumbs.map((b) => {
          if ('deps' in b || 'titleFn' in b) {
            ...
            if (tempTooltip && !isObservable(tempTooltip))
              tempTooltip = of(tempTooltip);
            return combineLatest([tempTitle, tempTooltip ?? of(null)]).pipe(
      ...
      switchMap((b) => from(b).pipe(combineLatestAll())),
    );
  }
}
